...
io.on('connection', function (socket) {
	...
	f.listen.on('listen', function (enable) {
		...
		if (pre_listen !== enable || default_connect) {
			// console.log(status)
			if (enable) {
				io.emit('server_status', "Running!!", 'lightgreen', 'Stop', 'stopp', 'Restart', 'restart')
				io.emit('load', false)

			}
			else if (status !== 'restart') {
				io.emit('server_status', "Not Run!", 'red', 'Start', 'start', 'Backup', 'backup')
				io.emit('load', false)
			}
			pre_listen = enable
	...
	socket.on('exe', function (action) {
		...
		ing_val = 'ing'
		io.emit('server_status', f.Upper(action) + 'ing...', 'orange', ing_val, '', ing_val, '', true)
		io.emit('load', true)
	})
